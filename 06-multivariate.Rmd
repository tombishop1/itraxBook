...
It's worth spending some time getting familiar with the documentation for `compositions`, and possibly the wider literature around compositional data analysis. Several different methods are provided depending on the nature of the compositional data - in these examples, we use the `acomp()` but other methods are available and may be more appropriate. This package also deals with the variety of different types of zero and missing values. 

in order to correctly identify the observations to their original data source, it is necessary to use row names that uniquely identify observations. For single scans this is not usually an issue --- the `depth` or `position` variable can be used. However, where a dataset is a composition of multiple cores, you may find that there are multiple observations for a particular depth where cores overlap.

```{r}
CD166_19_xrf %>%
  filter(qc == TRUE) %>%
  select(any_of(c(elementsList, "depth", "label"))) %>%
  mutate(id = paste0(label, "_", depth)) %>%
...
Principle component analysis (PCA) is a common method for exploring multivariate data. The plot is made with `ggfortify`, a wrapper for `ggplot2::biplot()`. 
...
bind_rows(tibble(depth = filter(CD166_19_xrf, qc == TRUE) %>% pull(depth),
                 group = dist(acomp(filter(CD166_19_xrf, qc == TRUE), 
                                    parts = names(CD166_19_xrf)[names(CD166_19_xrf) %in% elementsList])$clr) %>%
...
Constrained cluster analysis is helpful in identifying "break-points" in the sedimentary sequence. It forces each sample to remain in sequence, thus there are no recurring units along the stratigraphy. 

```{r constrained_clusters_1}
bind_rows(tibble(depth = filter(CD166_19_xrf, qc == TRUE) %>% pull(depth),
                 group = rioja::chclust(dist(acomp(filter(CD166_19_xrf, qc == TRUE), parts = names(CD166_19_xrf)[names(CD166_19_xrf) %in% elementsList])$clr)) %>%
                 cutree(k = 5) %>%
                 as.factor()), 
