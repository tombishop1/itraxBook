...
This can then be used to identify a representative sample from each compositional unit - see the following function:

```{r}
itrax_calibsamples <- function(original_data, transformed_data, n){

# function for first CA
firstCA <- function(original_data = original_data, transformed_data = transformed_data){
...
first_pass <- firstCA(original_data = original_data, transformed_data = transformed_data)

secondCA <- function(x, original_data = original_data, transformed_data = transformed_data){
  transformed_data[firstCA(original_data = original_data, transformed_data = transformed_data) %>%
                     select(uid, group) %>%
                       group_by(group) %>%
                       group_split() %>%
                       `[[`(x) %>% # this defines the group of interest
                       pull(uid),] %>%
    dist() %>%
    hclust(method = "ward.D2") %>%
    `[[`("labels") %>%
...
samples <- lapply(na.omit(unique(first_pass$group)), 
       function(x){secondCA(x = x, original_data = original_data, transformed_data = transformed_data)}) %>% 
  unlist()

CD166_19_xrf %>%
  filter(uid %in% samples)
}
```
...
```{r}
itrax_calibsamples(original_data = CD166_19_xrf, 
                   transformed_data = CD166_19_xrf_acomp[,c("Fe", "Mn", "Ca", "Ti")], 
                   n = 10) %>%
  select(uid, label, depth, everything())
```
