...
# check if non-CRAN repo's are installed, if not, install
install.packages("remotes", repos = "http://cran.uk.r-project.org")

if (!require("itraxR", character.only = TRUE)) {

  remotes::install_github("tombishop1/itraxR", method = "curl")
  library(itraxR)
}

if (!require("tidypaleo", character.only = TRUE)) {
  install.packages("remotes")
  remotes::install_github("paleolimbot/tidypaleo", method = "curl")
  library(tidypaleo)
}

if (!require("ordr", character.only = TRUE)) {
  install.packages("remotes")
  remotes::install_github("corybrunson/ordr", method = "curl")
  library(ordr)
}

# now check dependencies
# First specify the packages of interest
packages = c("kableExtra", "DiagrammeR", 
             "tidyverse" , "PeriodicTable", 
             "egg"       , "errors", 
             "forecast"  , "slider",
