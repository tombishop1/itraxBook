...
Where multivariate methods (cluster analysis, principle components analysis, correlation matricies) are required, it is usually necessary to transform data. This is because the statistical assumptions that underlie these methods are often not met when dealing with compositional data like that from an XRF core-scanner. There is no "right" or ideal way to deal with these issues, but a common method is to use a form of log transformation. Here we use a centred log transformation, which cannot be performed on zero values. 
...
tibble( depthmin = itrax_averaging(CD166_19_S1, 10)$depthmin,
        depthmax = itrax_averaging(CD166_19_S1, 10)$depthmax,
        Fe_mean  = itrax_averaging(CD166_19_S1, 10)$Ti,
        Fe_sd    = itrax_averaging(CD166_19_S1, 10, fun = sd)$Ti )
```
