...
The easiest way to do this is using a `tidyverse` style sequence of pipes that set the observations of faulty data as `NA` or create a new column (e.g. `qc`) that flags poor quality data so it can be excluded from some analyses.
...
read_spectra <- function(foldername, labeltext){
  itrax_restspectra(foldername = foldername,   
                    plot = FALSE) %>%
...
left_join(CD166_19_xrf %>%
            ...
            mutate(filename = filename %>%
                     str_split(pattern = "\\\\") %>%
                     sapply(., `[`, 7)), 
          bind_rows(read_spectra(foldername = "CD166_19_S1/CD166_19_S1/XRF Data.zip", 
                                 labeltext = "S1"), 
                    read_spectra(foldername = "CD166_19_S2/CD166_19_S2/XRF Data.zip", 
                                 labeltext = "S2"),
                    read_spectra(foldername = "CD166_19_S3/CD166_19_S3/XRF Data.zip", 
                                 labeltext = "S3")) %>%
...
```{r spectra_housekeeping, include=FALSE}
