...
The best way to quantify the uncertainties in the XRF data is in repeat measurements. The example dataset contains short sections of repeat measurements. An example might begin by reading in the repeat scan data, as below. 

```{r uncertainties}
CD166_19_S1_REP1 <- itrax_import("CD166_19_S1/CD166_19_S1_REP1/Results.txt") %>%
  select(any_of(c(elementsList, "position"))) %>%
  mutate(scan = "scan1")
...
It should then be combined into a single dataset. 
...
Finally, a function to calculate the uncertainties. The `errors` package is used to create an output that can combine both the value and the associated uncertainty. It is a really neat way of working with quantities. 

```{r}
library(errors)
S1_reps <- S1_reps %>%
  group_by(position) %>%
  summarise(across(any_of(elementsList), 
...
These can be plotted using the mean and standard deviation instead of a single value using `errors::errors_min()` and `errors::errors_max()` really easily. 

```{r}
S1_reps %>%
  select(Al, Si, Ti, Fe, position) %>%
  ggplot(aes(x = position, y = Fe)) +
  geom_ribbon(aes(ymin = errors_min(Fe), ymax = errors_max(Fe)), fill = "black") +
  scale_x_reverse()
```
