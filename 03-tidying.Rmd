...
A useful tool for investigating areas of high errors is to visualise the raw spectral data. This shows a heat map of the spectra, and can be used to identify areas where particular spectral lines appear and disappear along the core.

```{r}
spectra <- itrax_restspectra("CD166_19_S1/CD166_19_S1/XRF data/")
```
```{r include=FALSE}
rm(spectra)
```
...
CD166_19_xrf %>%
  ...
  mutate(in_mse_tolerance = ifelse(MSE <=2, FALSE, TRUE)) %>%
  rowwise() %>%
  mutate(validity = any(c(validity, in_slope_tolerance, in_cps_tolerance, in_mse_tolerance) == FALSE)) %>%
  select(-c(in_slope_tolerance, in_cps_tolerance, in_mse_tolerance)) %>%
  
  ggplot(data = ., aes(y = depth, x = `Mo inc`/`Mo coh`, col = validity)) + 
  geom_lineh(aes(group = 1)) +
  geom_rug(sides = "l", data = . %>% filter(validity == FALSE)) +
  scale_y_reverse()
```
